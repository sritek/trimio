 ...
 * - Reason field
 *
 * Note: Stylist changes are handled via the Edit Services dialog
 * to properly support multi-service appointments with per-service stylist assignments.
 */
...
export function RescheduleAppointmentDialog({
  ...
  }, [appointment.id, newDate, newTime, reason, rescheduleMutation, onOpenChange, onSuccess]);

  const handleOpenChange = useCallback(
    (newOpen: boolean) => {
      // Don't allow closing while mutation is pending
      if (!newOpen && rescheduleMutation.isPending) {
        return;
      }
      if (!newOpen) {
        setNewDate('');
    ...
    [onOpenChange, rescheduleMutation.isPending]
  );
  ...
  const hasChanges =
    newDate !== toDateString(appointment.scheduledDate) || newTime !== appointment.scheduledTime;
